...
public static class ServerSelector
{
    public static (Embed Embed, MessageComponent Components) Build(
        ConsoleFlow flow, string sessionId, IReadOnlyList<ServerEntry> servers, Localizer loc)
    {
        var embed = new EmbedBuilder()
            .WithTitle(loc.Get("selector.server.title"))
            .WithColor(Color.Purple)
            .Build();

        var menu = new SelectMenuBuilder()
            .WithCustomId($"{CustomIds.SelServer}{(int)flow}:{sessionId}")
            .WithPlaceholder(loc.Get("selector.server.placeholder"))
            .WithMinValues(1)
            .WithMaxValues(1);
