...
public static class CommandSelector
{
    ...
    public static (Embed Embed, MessageComponent Components) BuildForCommands(
        string sessionId, IReadOnlyList<CommandDefinition> commands, Localizer loc)
    {
        var embed = new EmbedBuilder()
            .WithTitle(loc.Get("selector.command.title"))
            .WithColor(Color.Purple)
            .Build();

        var menu = BuildMenu($"{CustomIds.SelCommand}{sessionId}", loc.Get("selector.command.placeholder"), commands);
    ...
    {
        var embed = new EmbedBuilder()
            .WithTitle(loc.Get("selector.punishment.title"))
            .WithColor(Color.DarkOrange)
            .Build();

        var menu = BuildMenu($"{CustomIds.SelAction}{sessionId}", loc.Get("selector.punishment.placeholder"), commands);
        var components = new ComponentBuilder().WithSelectMenu(menu).Build();
        return (embed, components);
    }
}
