...
public class AdminConsolePlugin : BasePlugin, IPluginConfig<AdminConsoleConfig>
{
    ...
    private void StartServicesLocked()
    {
        ...
        var localizer = new Localizer(ModuleDirectory, config.Language);
        ...
        var commands = new CommandService(_store, config.Integrations.PisexAdminSystem, localizer);
        ...
        var limiter = new RateLimiter(localizer);
        ...
        discord = new DiscordService(() => _config, handler, localizer);
        _discord = discord;
        ...
        Log.Info($"Plugin initialized. Storage: {(_store.Mode == StorageMode.Database ? "MySQL" : "JSON")}, Language: {localizer.Language}.");
    }
    ...
    private IDataStore BuildStore(AdminConsoleConfig config)
    {
        ...
        if (!string.IsNullOrWhiteSpace(db.Host) && !string.IsNullOrWhiteSpace(db.Database))
        {
            ...
            {
                Log.Warning($"MySQL unreachable ({ex.Message}) - falling back to JSON file storage. " +
