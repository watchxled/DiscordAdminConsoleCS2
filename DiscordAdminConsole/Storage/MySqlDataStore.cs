...
public class MySqlDataStore : IDataStore
{
    ...
    private DateTimeOffset _nextHealthAttempt;
    ...
    private async Task SeedAsync(MySqlConnection conn)
    {
        ...
        if (flagsCount == 0)
        {
            foreach (var (name, description) in new Dictionary<string, string>
                     {
                         [SystemFlags.PlayerKick] = "Kick players",
                         [SystemFlags.PlayerBan] = "Ban/unban",
                         [SystemFlags.PlayerMute] = "Mute/gag/silence",
                         [SystemFlags.ServerRcon] = "RAW RCON",
                     })
            {
                await using var cmd = new MySqlCommand(
    ...
    private async Task<bool> EnsureHealthyAsync()
    {
        ...
        lock (_healthLock)
        {
            if (DateTimeOffset.UtcNow < _nextHealthAttempt)
                return false;
            _nextHealthAttempt = DateTimeOffset.UtcNow + TimeSpan.FromSeconds(10);
        }
